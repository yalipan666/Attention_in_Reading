...
% 1.select tagging response sensors (onlu from occipital); 2.get 2D coherence; 3.plot it!

function Coh_RIFT(sid)
%%% set paths
server = 1; % run analysis on the server or local computer
[rootdir,cmap] = Get_Paths(server); %get the root dir and the colormap of cbrewer(RdBu)
...
EpochType = {'PreTarg';'Targ';'PosTarg'}; % epochs of interest; [PreTarg_parafoveal;Targ_parafoveal;PosTarg_parafoveal;Targ_foveal]
...
for ep = 1:length(EpochType) % epoch index for sensor selection;{'PreTarg','Targ','PosTarg'};
    ...
    trlid     = epoch.trialinfo(:,tag_col)==loc2tar(ep); %select the epochs of interest
    cfg         = [];
    cfg.trials  = find(trlid);
...
%%%%%%%%%%%%%%%%%%%%%%%% 3. Plotting figures %%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figtitle = ['TagCoh_' num2str(sid)];
if any(TagCoh.PreTarg_SigTagSens{1,2}==0)
    figtitle = ['NoSig_' figtitle];
end
h = figure('Name',figtitle,'color',[1 1 1]);
for ep = 1:length(EpochType)
    tf = whichfreq(ep);
    %%%% coh plot
    eval(['tmpdata = nanmean(TagCoh.' EpochType{ep} '_Coh(:,:,1,:),3);']);%%% freq*time*sub*cond
    tmpcc = [1 2 3];
    for cc = 1:length(condnm)+1
        if ismember(cc,tmpcc)
            condid = find(tmpcc==cc);
            eval(['rt = TagCoh.' EpochType{ep} '_RT(:,condid)./1000;']);
            rt = nanmean(rt);
            loc = tmpcc(condid)+4*(ep-1);
            plotdata = squeeze(tmpdata(:,:,:,condid));
            subtitle = [EpochType{ep} 'et,' num2str(tagfreq(tf)) 'Hz,' TagCoh.CondName{condid}];
            subplot(4,4,loc);
            pcolor(TagCoh.time, TagCoh.freq, plotdata); colorbar;
        else
            eval(['rt = TagCoh.' EpochType{ep} '_RT(:,1)./1000;']); %% for the difference figure, use the RT from 'all' cond
            rt = nanmean(rt);
            loc = cc+4*(ep-1);
            if cc == 4
                idx1 = 2; idx2 = 3;
        ...
        hold on;
        for p = 1:length(tagfreq)
            plot([-0.5 0.5],[tagfreq(p) tagfreq(p)],'-.k','LineWidth',2)
        end
        ...
        title(subtitle,'FontWeight','bold','FontSize',12);
    end
end
ScSz = [0 0 1300 800];
set(gcf,'Position',ScSz);
saveas(h,[PPath.ResPath figtitle],'bmp');
end
